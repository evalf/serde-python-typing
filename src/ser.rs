...
use pyo3::{Python, PyResult, PyErr, AsPyRef};
use pyo3::types::{PyAny, PyBool, PyInt, PyFloat, PyComplex, PyTuple, PySet, PyFrozenSet, PyString, PyBytes, PyType};
...
macro_rules! is_instance {
  ($py:expr, $val:expr, T:$typ:ty) => { pytry!($py, $py.is_instance::<$typ, _>($val)) };
  ($py:expr, $val:expr, O:$typ:expr) => { pytry!($py, $typ.is_instance($val)) };
}
...
macro_rules! get_type_obj {
  ($py:expr, T:$typ:ty) => { $py.get_type::<$typ>() };
  ($py:expr, O:$typ:expr) => {{ let typ: &PyType = $typ; typ }};
}

macro_rules! check_is_instance {
  ($py:expr, $val:expr, T:$typ:ty) => {
    let val: &PyAny = $val;
    raise_if!(!is_instance!($py, val, T:$typ),
              $py,
              ValueError,
              format!("expected an instance of {} but got an instance of {}",
                      match get_type_name(get_type_obj!($py, T:$typ)) { Ok(v) => format!("`{}`", v), Err(_) => "?".to_string() },
                      match get_type_name(val.get_type()) { Ok(v) => format!("`{}`", v), Err(_) => "?".to_string() }));
  };
  ($py:expr, $val:expr, O:$typ:expr) => {
    let val: &PyAny = $val;
    raise_if!(!is_instance!($py, val, O:$typ),
              $py,
              ValueError,
              format!("expected an instance of {} but got an instance of {}",
                      match get_type_name(get_type_obj!($py, O:$typ)) { Ok(v) => format!("`{}`", v), Err(_) => "?".to_string() },
                      match get_type_name(val.get_type()) { Ok(v) => format!("`{}`", v), Err(_) => "?".to_string() }));
  };
}

fn serialize_seq<S: Serializer>(py: Python, serializer: S, seq: &PyAny, typ: &Type) -> Result<S::Ok, S::Error> {
  let mut serseq = serializer.serialize_seq(match seq.len() { Ok(len) => Some(len), Err(_) => None })?;
...
impl<'a> Serialize for Wrapper<'a> {
  fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error>
  {
    match self.typ {
      ...
      Type::Float => {
        serializer.serialize_f64(if is_instance!(self.py, self.val, T:PyFloat) {
      ...
      Type::Complex => {
        if is_instance!(self.py, self.val, T:PyComplex) {
      ...
      Type::Dict(ktyp, vtyp) => {
        let mut sermap = serializer.serialize_map(match self.val.len() { Ok(len) => Some(len), Err(_) => None })?;
        for item in pytry!(self.py, pytry!(self.py, self.val.call_method0("items")).iter()) {
          ...
          sermap.serialize_entry(&Wrapper { py: self.py, typ: ktyp, val: pytry!(self.py, item.get_item(0)) },
                                 &Wrapper { py: self.py, typ: vtyp, val: pytry!(self.py, item.get_item(1)) })?;
        }
        sermap.end()
      },
